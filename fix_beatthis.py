...
for root, dirs, files in os.walk(beatthis_path):
    for file in files:
        if file.endswith('.py'):
            ...
            # Fix union types in variable annotations
            content = re.sub(r': ([A-Za-z_\.]+) \| None', r': Optional[\1]', content)
            content = re.sub(r'([a-z_]+): ([A-Za-z_\.]+) \| None', r'\1: Optional[\2]', content)
            
            # Fix union types in function signatures (str | torch.device, etc.)
            content = re.sub(r'(def \w+\([^:]+): ([A-Za-z_\.]+) \| ([A-Za-z_\.]+))', r'\1: Union[\2, \3]', content)
            content = re.sub(r' -> ([A-Za-z_\.]+) \| None', r' -> Optional[\1]', content)
            
            # Add imports if needed
            if 'from typing import Optional, Union' not in content and ('Optional' in content or 'Union' in content):
